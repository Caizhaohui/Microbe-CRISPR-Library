# filename: Cas9_knockout_designer_v5.py
# Version 5.0 - 删除长度百分比模式
# 主要改进（相较V4）：
#   --del_length 支持CDS百分比格式，如 '50%:90%' 表示删除每个基因CDS长度的50%-90%
#   每个基因按其实际CDS长度动态计算 min_del / max_del
#   向后兼容：不带'%'时仍按绝对bp工作（如 '500:1000'）
#   新增 ConfigParser.parse_del_length_param() 解析两种格式
#   运行时需传 --restriction_site GGTCTC GAAGAC 启用酶切位点过滤
...
class DesignConfig:
    ...
    barcode_gc_min: float = 0.35  # 真菌优化值（原0.30）
    barcode_gc_max: float = 0.65  # 真菌优化值（原0.70）
    barcode_repeat_threshold: int = 5  # 长重复序列阈值（原6）
    neighbor_distance_threshold: int = 1500  # 邻近基因保护距离（真菌优化，原1000）
    max_oligo_length: Optional[int] = None  # 限制最终合成oligo最大长度
    pam_len: int = 3  # PAM长度（由--pam动态设置）
    del_pct_mode: bool = False    # V5新增：是否使用CDS百分比模式
    min_del_pct: float = 0.0      # CDS最小删除比例（0-1）
    max_del_pct: float = 0.0      # CDS最大删除比例（0-1）
...
class ConfigParser:
    ...
    @staticmethod
    def parse_del_length_param(param_str: str) -> Tuple[bool, float, float]:
        """
        解析删除长度参数，支持两种格式：
          绝对长度：'100:220'  → (False, 100.0, 220.0)
          CDS百分比：'50%:90%' → (True, 0.50, 0.90)
        返回: (is_pct_mode, min_val, max_val)
        """
        is_pct = '%' in param_str
        ...
        if len(parts) == 1:
            ...
            raise ValueError(f"无效的del_length格式: '{param_str}'")
        v_min, v_max = min(v1, v2), max(v1, v2)
        if is_pct:
            return True, v_min / 100.0, v_max / 100.0
        else:
...
class CRISPRDesigner:
    ...
    def design_knockout_for_gene(self, gene: Gene, gene_index: int, all_genes: List[Gene], pam: str, used_barcodes: Set[str], restriction_sites: Optional[List[str]]) -> List[KnockoutDesignResult]:
        ...
        # V5：支持CDS百分比模式的动态删除长度
        if self.config.del_pct_mode:
            min_del = max(1, int(cds_len * self.config.min_del_pct))
            max_del = int(cds_len * self.config.max_del_pct)
        else:
        ...
        for sgrna in all_sgrnas:
            ...
            # 预计算此sgRNA的有效del_len上界（避免无效迭代）
            max_del_from_bounds = min(2 * (cut_site - cds_min), 2 * (cds_max - cut_site))
            effective_max_del = min(max_del, max_del_from_bounds)
            if effective_max_del < min_del:
                continue
            
            for del_len in range(min_del, effective_max_del + 1):
                del_half = del_len >> 1
                del_start = cut_site - del_half
                del_end = cut_site + del_half + (del_len & 1)

                if del_start < cds_min or del_end > cds_max: continue
                ...
                if best_candidate_for_this_sgrna is None or current_sort_key < best_candidate_for_this_sgrna['sort_key']:
                    best_candidate_for_this_sgrna = {
                        ...
                        'del_len': del_len, 'arm_len': preferred_arm_len, 'sort_key': current_sort_key
                    }
...
def run_knockout_pipeline(args, config, genes, genome_processor):
    ...
    for i, gene in enumerate(genes):
        if (i + 1) % 500 == 0:
            ...
            logging.info(f"正在处理基因 {i+1}/{len(genes)}... 已用时 {elapsed:.1f}s")
        ...
        gene_design_counts[gene.id] = len(results)
        if results:
...
def main():
    ...
    parser.add_argument("--del_length", type=str, default="100:300",
                        help="删除长度范围，支持两种格式：\n"
                             "  绝对长度 (bp)：'最小:最大'，如 '500:1000'\n"
                             "  CDS百分比：'最小%%%%:最大%%%%'，如 '50%%%%:90%%%%'（按每个基因CDS长度动态计算）\n"
                             "  默认: 100:300 bp")
    ...
    try:
        ...
        _del_pct_mode, _del_v1, _del_v2 = ConfigParser.parse_del_length_param(args.del_length)
        config.del_pct_mode = _del_pct_mode
        if _del_pct_mode:
            config.min_del_pct, config.max_del_pct = _del_v1, _del_v2
            del_len_desc = f"CDS的 {_del_v1*100:.0f}%-{_del_v2*100:.0f}%（按基因动态计算）"
        else:
            config.min_del, config.max_del = int(_del_v1), int(_del_v2)
            del_len_desc = f"{config.min_del}-{config.max_del} bp"
