# filename: Cas9_knockout_designer_v4.py
# Version 4.0 - 酶切位点检查修复版本
# 主要改进（相较V3）：
#   修复 check_final_oligo 逻辑——模板固定区中已有 BsaI/BbsI 不应触发过滤；
#   新增 parse_template_segments / check_junctions_for_restriction_sites：
#   仅检查可变区（sgRNA_rc/upstream_arm/barcode/downstream_arm）与固定区的拼接边界窗口，
#   拼接点窗口宽度 = max(len(site)) - 1，确保新产生的位点被捕获；
#   运行时需传 --restriction_site GGTCTC GAAGAC 启用过滤。
