# filename: knockout_designer_v2.py
# Version 2.0 - 针对真菌（嗜热毁丝霉）优化版本
# 主要改进：物种特异性参数、配置系统、barcode GC范围参数化
...
class CRISPRDesigner:
    ...
    def design_knockout_for_gene(self, gene: Gene, gene_index: int, all_genes: List[Gene], pam: str, used_barcodes: Set[str], restriction_sites: Optional[List[str]]) -> List[KnockoutDesignResult]:
        ...
        for design_candidate in best_design_per_sgrna:
            ...
            arm_len = 0
            for try_arm in self.config.arm_search_order:
...
def main():
    parser = argparse.ArgumentParser(
        description="基因敲除 (Knockout) 文库设计工具 (Cas9) - V2针对真菌优化版本", 
    ...
    parser.add_argument("--HR_len", type=str, default="100:80", 
                        help="同源臂长度。格式为 '首选:最小' (真菌优化默认: 100:80)。")
    parser.add_argument("--del_length", type=str, default="100:300", 
                        help="删除长度范围 '最小:最大' bp (真菌优化默认: 100:300)。")
    ...
    try:
        ...
        if SPECIES_CONFIG_AVAILABLE:
            try:
                ...
                if args.HR_len == "100:80" and hasattr(species_cfg, 'homolog_arm_range'):
        ...
        logging.info(f"  同源臂范围: {min(config.arm_search_order)}-{max(config.arm_search_order)} bp")
        logging.info(f"  删除长度: {config.min_del}-{config.max_del} bp")
